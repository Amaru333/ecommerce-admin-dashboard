...
import { IAdminData } from "@/interfaces/adminInterfaces";
...
const Navbar = () => {
  ...
  const adminData: IAdminData | undefined = queryClient.getQueryData(["admin-details"]);

  console.log(adminData, "ADMIN-DATA");

  return (
    <div className="p-4 flex items-center justify-between mr-4 border-b border-slate-800">
      <NavigationMenu>
        <NavigationMenuList className="w-fit">
          {NAVBAR_ITEMS.map((item) => (
            <NavigationMenuItem key={item.name} className={`border-b ${pathname?.includes(item.path) ? "border-slate-100" : "border-transparent"}`}>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="flex items-center gap-x-4">
          <Image src={adminData?.profile_image ?? ""} alt={adminData?.name ?? ""} width={20} height={20} className="rounded-full" />
          {adminData?.name}
        </DropdownMenuTrigger>
        <DropdownMenuContent>
